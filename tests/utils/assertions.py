...
These functions encode common checks that appear in multiple test files,
keeping individual tests focused on their specific scenario logic.

Categories
----------
- Status code assertions (assert_ok, assert_created, assert_no_content, …)
- Security assertions (assert_no_sensitive_fields)
- Shape assertions (assert_user_shape, assert_token_shape, …)
"""
...
# ---------------------------------------------------------------------------
# Sensitive fields — must never appear in any API response
# ---------------------------------------------------------------------------
...
def assert_unauthorized(resp: Response, msg: str = "") -> None:
...
def assert_no_sensitive_fields(data: dict | list) -> None:
    ...
    Accepts a single dict or a list of dicts (e.g. a list-users response).
    """
...
def assert_user_shape(data: dict) -> None:
    ...
    assert isinstance(data["username"], str) and data["username"], "username must be a non-empty string"
    assert isinstance(data["email"], str) and "@" in data["email"], "email must be a valid string"
...
def assert_error_detail(resp: Response, fragment: str) -> None:
    ...
    Works for both {"detail": "string"} and FastAPI validation error
    lists {"detail": [{"msg": "..."}]}.
    """
    body = resp.json()
